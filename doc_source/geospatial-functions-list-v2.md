...
This topic lists the ESRI geospatial functions that are supported in Athena engine version 2\. Currently, Athena engine version 2 is supported in the Asia Pacific \(Mumbai\), Asia Pacific \(Seoul\), Asia Pacific \(Singapore\), Asia Pacific \(Sydney\), Asia Pacific \(Tokyo\), Europe \(Ireland\), Europe \(London\), Europe \(Paris\), US East \(N\. Virginia\), US East \(Ohio\), US West \(N\. California\), and US West \(Oregon\) Regions\. For information about Athena engine versions, see [Athena Engine Versioning](engine-versions.md)\.
