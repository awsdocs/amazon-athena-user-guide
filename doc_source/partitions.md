...
To create a table that uses partitions, you must define it during the `` statement\. Use `PARTITIONED BY` to define the keys by which to partition data, as in the following example\. `LOCATION` specifies the root location of the partitioned data\.
