...
+ **SSL\_CERT\_FILE** – For internal use only\. The default is `/tmp/cacert.pem`\. This parameter should not be changed\.
+ **GOOGLE\_APPLICATION\_CREDENTIALS** – For internal use only\. The default is `/tmp/service-account.json`\. This parameter should not be changed\.
