...
+ Athena does not support querying the data in the `GLACIER` or `DEEP GLACIER` storage classes\. It ignores objects that, based on an Amazon S3 lifecycle policy, have been transitioned to the `GLACIER` storage class\. Querying objects in the `DEEP GLACIER` storage class produces the error message The operation is not valid for the object's storage class\. 
